...
ELB_NAME_LAMP=$(bash elbs-create.sh ${PROJECT} ${ENVIRONMENT} lamp "${SUBNET_ID_PUBLIC} ${SUBNET_ID_PUBLIC_ALT}" ${SECURITY_GROUP_ID_ELB} ${TARGET_GROUP_ARN_LAMP} | grep "aws_ElbName" | cut -f2)
