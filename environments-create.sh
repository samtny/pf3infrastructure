...
ELB_NAME_LAMP=$(bash elbs-create.sh $PROJECT $ENVIRONMENT lamp $SUBNET_ID_PUBLIC $SECURITY_GROUP_ID_ELB | grep "aws_ElbName" | cut -f2)
echo -e "aws_ElbNameLamp:\t$ELB_NAME_LAMP"

# instances

INSTANCE_ID_LAMP=$(bash instances-create.sh $PROJECT $ENVIRONMENT lamp $VPC_ID $SECURITY_GROUP_ID_LAMP $SUBNET_ID_PRIVATE $ELB_NAME_LAMP $EC2_INSTANCE_TYPE_DEFAULT $EC2_AMI_DEFAULT '--block-device-mappings [{"DeviceName":"/dev/xvdf","Ebs":{"VolumeSize":80}}]' | grep "aws_InstanceId" | cut -f2)
