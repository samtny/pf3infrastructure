...
SUBNET_ID_PRIVATE=$(aws ec2 describe-subnets --output text --filters Name=tag:Name,Values=$PROJECT-private | grep "^SUBNETS" | cut -f8)
echo -e "aws_SubnetIdPrivate:\t$SUBNET_ID_PRIVATE"

SUBNET_ID_PUBLIC=$(aws ec2 describe-subnets --output text --filters Name=tag:Name,Values=$PROJECT-public | grep "^SUBNETS" | cut -f8)
